...
//import org.dkpro.tc.features.EmotionLexiconFeatureExtractor;
...
public class Classifier implements Constants {
	...
	public static final String corpusFilePathTest = "src/main/resources/data/wassa2018/testR/"; 
	...
	public static void runExperiment() throws Exception {
		for (int emotionPosInArray = 0; emotionPosInArray < 1; emotionPosInArray++) {
			...
			builder.experiment(ExperimentType.TRAIN_TEST, "trainTest-" + emotion[emotionPosInArray]) // workflow: train test
					.dataReaderTrain(readerTrain).dataReaderTest(readerTest).preprocessing(getPreprocessing()) // preprocessing
					.featureSets(getFeatureSet()).learningMode(LearningMode.SINGLE_LABEL) // classification single-label, da pro Label eigener Classifikator
	...
	private static AnalysisEngineDescription getPreprocessing() throws ResourceInitializationException, IOException {
		...
		return createEngineDescription(createEngineDescription(BreakIteratorSegmenter.class),
				...
				createEngineDescription(StanfordPosTagger.class, StanfordPosTagger.PARAM_LANGUAGE, "en"));
